...
test("Referrals work (NEAR)", async (test) => {
  ...
  test.log(`market: ${preMarketBalance} -> ${postMarketBalance}`); // 10.00 -> 10.24
  test.log(`alice: ${preAliceBalance} -> ${postAliceBalance}`); // 12.96 -> 22.72
  test.log(`bob: ${preBobBalance} -> ${postBobBalance}`); // 20.00 -> 20.00
  test.log(`carol: ${preCarolBalance} -> ${postCarolBalance}`); // 20.00 -> 9.99
  ...
  test.true(
    diffCheck(
      ...
      preMarketBalance,
      nearToBn("0.40"),
      nearToBn("0.20") // -> storage + yocto + something else (not exact)
      // nearToBn("0.10"),
